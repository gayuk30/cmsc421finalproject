...
def extract_transcript_segment(transcript_list, start_time, end_time):
    ...
    last_possible_start = max(item['start'] for item in transcript_list) if transcript_list else 0
    if start_time > last_possible_start or end_time > last_possible_start + 600:
        raise ValueError("Specified times are out of bounds for the video transcript.")
...
def summarize_youtube_video(video_url):
    ...
    if use_timestamps in ['yes', 'y', 'Yes']:
        try:
            ...
            transcript_segment = extract_transcript_segment(transcript_list, start_time, end_time)
            if not transcript_segment:
                return "No text available for the specified time segment."
